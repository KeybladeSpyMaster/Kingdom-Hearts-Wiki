...
(function () {
    ...
    function tileId(title) {
        return 'khwiki.' + md5(title.replace(/_/g, ' '));
    }
            ...
            function doStuff() {
                ...
                WinJS.Application.onsettings = function (e) {
                    e.detail.applicationcommands = {
                        ...
                        options: {
                            title: mediaWiki.message('menu-options').plain(),
                            href: "/options.html"
                        },
                        help: {
                            title: mediaWiki.message('menu-help').plain(),
                            href: "/help.html"
                        }
                    };
                ...
                $('#pinCmd').click(function () {
                    var lang = state.current().lang,
                        ...
                        shortName = title,
                        displayName = title + ' - Kingdom Hearts Wiki',
    ...
    function getLanguageLinks(lang, title) {
        var url = 'http://www.khwiki.com/api.php';
    ...
    searchPane.addEventListener("suggestionsrequested", function (e) {
        ...
        var suggestionUri = "http://www.khwiki.com/api.php?action=opensearch&namespace=0&suggest=&search=";
    ...
    function doLoadPage(lang, title) {
        ...
        $('#spinner').show();
        $.ajax({
            url: 'http://www.khwiki.com/api.php',
            data: {
                action: 'mobileview',
                page: title,
                sections: 'all',
            ...
            success: function (data) {
                ...
                */
                ...
                // Image fix for KHWiki.com
                $('img').each(function () {
                    var src = $(this).attr('src');
                        if (src.indexOf('/') === 0) {
                            this.src = src.replace('/', 'http://www.khwiki.com/');
                        }
                });

                //Remove current tab from Template:Suite
                $('ul.suiteul li.suite_tab suite_tab_selected').remove();


                // Make additional div from Template:Suite transparent
                $('#suitediv').each(function () {
                    document.getElementById("suitediv").style.background = "none";
                });
    ...
    dataTransferManager.addEventListener("datarequested", function (e) {
        ...
        if (selection.isCollapsed) {
            ...
            request.data.properties.title = title + ' - Kingdom Hearts Wiki';
            request.data.properties.description = 'Link to article at KHWiki'; // @fixme l10n
        } else {
            // Active selection;
            ...
            request.data.properties.title = 'Content from ' + title + ' - The <i>Kingdom Hearts</i> Wiki';
            request.data.properties.description = 'Text from Kingdom Hearts Wiki article'; // @fixme l10n
        }
    });
    ...
    function articleUrl(lang, title) {
        ...
        return 'http://www.khwiki.com/' + encodeURIComponent(title.replace(/ /g, '_'));
    }
    ...
    //Featured Items, from the original Wikipedia app
    ...
    function insertWikiHtml(target, html) {
        ...
        $div.find('img').each(function () {
            ...
            var $img = $(this),
                src = $img.attr('src');
            if (src.substr(0) == '/') {
                $img.attr('src', 'http://www.khwiki.com/' + src);
            }
        });
    ...
    //NON-FUNCTIONING LOGIN SCRIPT
    //function Login (logname, logpassword) { // clears boxes if username or password is wrong
    //    var url = "http://www.khwiki.com/api.php";
    //    var logtoken;
    ...
    //        url: url,
    //        type: POST,
    //        data: {                     // deliver login + password to api using ajax
    //            action: 'login',
    //            lgname: logname,
    //            lgpassword: logpassword
    //        },
    //        success: function (data) { // callback function passed as parameter, called on success
    //            lgtoken = data.login.token; // extract login token
    //        }
    //    });

    //    $.ajax({  //Resend data with login token
    //        url: url,
    //        type: POST,
    //        data: { 
    //            action: 'login',
    ...
    //                $('#loggedInAs').text("ERROR: " + code);
    //            }
    //            else {
    //                $('#loggedInAs').text("Logged In As " + lgname);
    //            };
    //            return data;
    //        }
    //    });
    ...
    function initHub(lang) {
        ...
        //            image = $imgs.attr('src');
        //            if (image.substr(0, 2) == '/') {
        //                image = 'http://www.khwiki.com' + image;
        //            }
        ...
        getRecentChanges(lang).then(function (recentchanges, err) {
            ...
            recentchanges.forEach(function (change) {
                //if (change.ns == 2 && change.type == 'edit') {
                if (change.type == 'edit') {
                    nItems++;
                    $('#spinner').hide();
                    list.push({
    ...
    function sizeContent() {
        ...
        if (window.innerWidth <= 700) {
            ...
            $('#content').scrollTop(0); // avoid being scrolled off into nothingness
        ...
        if ($('#hub').is(':visible')) {
            ...
            $('#semanticZoomer').css('height', (h - 140) + 'px')
            $('#subcontent').css('height', (h - 180) + 'px');
        }
